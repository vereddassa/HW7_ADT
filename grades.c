...
int student_clone(void *element, void **output) {
	...
	if ((out->course_list)==NULL) {
		free (out->student_name);
		free (out);
		return 1;
	}
	...
	while (it!=NULL) {
		if (list_get(it)==NULL) {
			list_destroy (out->course_list);
			free (out->student_name);
			free (out);
			return 1;
		}
		int push =list_push_back(out->course_list,list_get(it));
		if (push !=0) {
			list_destroy (out->course_list);
			free (out->student_name);
			free (out);
 ...
 * @return We return 0 if the id is in the linked list ,1 if not,
 * and search_fail for an error , in case of an error set out to 
 * null
*/
 ...
 * @return 0 if the course is in the course list, 1 if not,
 * and search_fail for an error
*/
...
int grades_add_student (grades_t *grades, const char *name, int id) {
	...
	int search = list_search_id (grades ,id ,out);
	if (search == 0 || search == search_fail) {
		return 1;
	}
...
int grades_add_grade (grades_t *grades,
	...
	/* check data validation */
	int search = list_search_id(grades,id ,out);
	if (search || search == search_fail) {
		return 1;
	}
	int course_search = list_search_course(*out,name);
	if (!course_search || course_search == search_fail) { 
		return 1;
	}
...
float grades_calc_avg (struct grades *grades, int id, char **out) {
	...
	int search = list_search_id(grades , id, elm);
	if (search || search == search_fail) {
		*out = NULL;
		return -1;
	}
...
int grades_print_student (struct grades *grades, int id) {
	...
	student_t *test = NULL;
	student_t **el =&test;
	int search = list_search_id(grades , id, el);
	if (search || search == search_fail) {
		return -1;
	}
...
int grades_print_all (struct grades *grades) {
	...
	while (it!=NULL) {
		...
		grades_print_student(grades ,elm->id);
		it = list_next(it);
	}
	return 0 ;
}
